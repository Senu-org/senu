...
module Transactions = {
  module Transfer = {
    @genType
    let processEvent: EventFunctions.eventProcessor<Types.Transactions.Transfer.event> = EventFunctions.makeEventProcessor(
      ~register=(Types.Transactions.Transfer.register :> unit => Internal.eventConfig),
    )
...
->(Utils.magic: Types.Transactions.Transfer.eventArgs => Internal.eventParams)

      EventFunctions.makeEventMocker(
        ~params,
        ~mockEventData,
        ~register=(Types.Transactions.Transfer.register :> unit => Internal.eventConfig),
      )->(Utils.magic: Internal.event => Types.Transactions.Transfer.event)
    }
  }

  module Transaction = {
    @genType
    let processEvent: EventFunctions.eventProcessor<Types.Transactions.Transaction.event> = EventFunctions.makeEventProcessor(
      ~register=(Types.Transactions.Transaction.register :> unit => Internal.eventConfig),
    )
    ...
    let createMockEvent = args => {
      ...
      let params = 
      {
       ...
       currency: currency->Belt.Option.getWithDefault("foo"),
      }
->(Utils.magic: Types.Transactions.Transaction.eventArgs => Internal.eventParams)

      EventFunctions.makeEventMocker(
        ~params,
        ~mockEventData,
        ~register=(Types.Transactions.Transaction.register :> unit => Internal.eventConfig),
      )->(Utils.magic: Internal.event => Types.Transactions.Transaction.event)
    }
  }
