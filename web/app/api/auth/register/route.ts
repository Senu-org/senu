...
import type { RegisterUserRequest, RegisterUserResponse, ApiResponse, User } from '@/lib/types'
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      ...
      } as ApiResponse<RegisterUserResponse & { token: string; message: string }>,
      { status: 201 }
    )
      ...
      return NextResponse.json(
        { 
          ...
          error: { 
            ...
            message: 'Invalid phone number format. Please use format: +50688881111',
            timestamp: new Date() 
          } 
        } as ApiResponse<null>,
        { status: 400 }
