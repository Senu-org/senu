...
const TRANSACTION_STATE_TRANSITIONS: Record<TransactionStatus, TransactionStatus[]> = {
  ...
  [TransactionStatus.PAYMENT_CONFIRMED]: [TransactionStatus.BLOCKCHAIN_PENDING, TransactionStatus.FAILED],
  [TransactionStatus.BLOCKCHAIN_PENDING]: [TransactionStatus.BLOCKCHAIN_CONFIRMED, TransactionStatus.FAILED],
  [TransactionStatus.BLOCKCHAIN_CONFIRMED]: [TransactionStatus.WITHDRAWAL_PENDING, TransactionStatus.COMPLETED],
  [TransactionStatus.WITHDRAWAL_PENDING]: [TransactionStatus.COMPLETED, TransactionStatus.FAILED],
...
const RETRY_CONFIG = {
  ...
  retryableErrors: ['ONRAMP_SERVICE_UNAVAILABLE', 'BLOCKCHAIN_TIMEOUT', 'INTERNAL_SERVER_ERROR']
}

// Exchange rate and fee configuration (mock for now)
const EXCHANGE_RATES = {
  USD_CRC: 520.50, // Example rate
  USD_NIC: 36.80   // Example rate
}

const FEE_STRUCTURE = {
  platform_fee_percent: 2.5,
  onramp_fee_fixed: 3.00,
...
export class TransactionService {
  ...
  static async createTransaction(
    ...
    try {
      ...
      const exchangeRate = country === 'CR' ? EXCHANGE_RATES.USD_CRC : EXCHANGE_RATES.USD_NIC
      ...
      return {
        ...
        payment_url: this.generateMockPaymentUrl(transactionData.id, request.onramp_provider)
      }
    } catch (error) {
   ...
   */
  static async getTransactionStatus(transactionId: string, userPhone: string): Promise<TransactionStatusResponse> {
    try {
      ...
      const transactionData = this.mapDbToTransaction(transaction)
      const estimatedCompletion = this.estimateCompletionTime(transactionData.status)
   ...
   */
  static async retryTransaction(transactionId: string, userPhone: string): Promise<Transaction> {
    try {
      // Set user context for RLS
      await setUserContext(userPhone)
  ...
  static async updateTransactionStatus(
    ...
    try {
      // Get current transaction
      const { data: currentTransaction, error: fetchError } = await supabaseServer
        .from(TABLES.TRANSACTIONS)
        .select('*')
        .eq('id', transactionId)
        .single()
      ...
      const allowedTransitions = TRANSACTION_STATE_TRANSITIONS[currentStatus] || []
  ...
  static async processTransactionAsync(transactionId: string): Promise<void> {
    try {
      ...
      try {
        ...
        await this.handleTransactionFailure(transactionId, processError as Error)
      }
    } catch (error) {
   ...
   */
  private static async processOnRampPayment(transaction: Transaction): Promise<void> {
    console.log(`Processing on-ramp payment for transaction ${transaction.id}`)
    
    // Update to payment pending
    await this.updateTransactionStatus(transaction.id, TransactionStatus.PAYMENT_PENDING)
   ...
   */
  private static async processBlockchainTransfer(transaction: Transaction): Promise<void> {
    console.log(`Processing blockchain transfer for transaction ${transaction.id}`)
    
    // Update to blockchain pending
    await this.updateTransactionStatus(transaction.id, TransactionStatus.BLOCKCHAIN_PENDING)
   ...
   */
  private static async processOffRampWithdrawal(transaction: Transaction): Promise<void> {
    console.log(`Processing off-ramp withdrawal for transaction ${transaction.id}`)
    
    // Check if receiver wants automatic withdrawal
    const receiver = await AuthService.getUserByPhone(transaction.receiver_phone)
    if (!receiver) {
    ...
    await this.updateTransactionStatus(transaction.id, TransactionStatus.WITHDRAWAL_PENDING)
   ...
   */
  private static async handleTransactionFailure(transactionId: string, error: Error): Promise<void> {
    try {
      console.log(`Handling failure for transaction ${transactionId}:`, error.message)
   ...
   */
  private static async validateTransactionLimits(senderPhone: string, amountUsd: number): Promise<void> {
    ...
    const dailyTotal = (recentTransactions || []).reduce((sum, tx) => sum + tx.amount_usd, 0)
   ...
   */
  private static generateMockPaymentUrl(transactionId: string, provider: string): string {
   ...
   */
  private static async simulateAsyncProcess(delayMs: number, failureRate: number = 0): Promise<void> {
  ...
  private static mapDbToTransaction(dbTransaction: any): Transaction {
    return {
      ...
      completed_at: dbTransaction.completed_at ? new Date(dbTransaction.completed_at) : undefined,
   ...
   */
  static async getUserTransactions(userPhone: string, limit: number = 50): Promise<Transaction[]> {
    try {
      await setUserContext(userPhone)
