...
export class NotificationHandler {
  ...
  async handleTransactionStatusMenuSelection(phoneNumber: string, userName?: string) {
    await this.botService.sendMessage(phoneNumber, "Your recent transactions... (transaction status not implemented yet)");
    // This will be updated when transaction status endpoint is implemented
  }

  // Handle help command
  async handleHelpCommand(phoneNumber: string, userName?: string) {
  ...
  async handleHelpMenuSelection(phoneNumber: string, userName?: string) {
    await this.botService.sendMessage(phoneNumber, "Here are the available commands:\n/send - Send money\n/status - Check transaction status\n/balance - Check your balance\n/menu - Show main menu\n/help - Show this help message");
  }

  // Handle unknown command
  async handleUnknownCommand(phoneNumber: string, userName?: string) {
  ...
  async handleTransactionStatusError(phoneNumber: string, error: any) {
  ...
  formatTransactionStatusForDisplay(transactions: any[]) {
