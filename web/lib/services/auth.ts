...
export class AuthService {
   ...
   */
  static async registerUser(phone: string, name: string, country: 'CR' | 'NI'): Promise<User> {
    try {
    ...
    /**
   * Register a new user by phone number (alias for registerUser)
   */
  static async register(phoneNumber: string, name: string, country: string): Promise<User> {
    return this.registerUser(phoneNumber, name, country as 'CR' | 'NI');
  }
  ...
  static async isUserRegistered(phoneNumber: string): Promise<boolean> {
    try {
      ...
      console.error('AuthService.isUserRegistered error:', error)
      return false;
    }
  }
}